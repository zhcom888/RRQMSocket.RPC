...
namespace RRQMSocket.RPC.TouchRpc
{
    public partial class RpcActor
    {
        ...
        /// </summary>
        public Action<FileTransferStatusEventArgs> OnFileTransfered { get; set; }

        /// <summary>
        /// 在文件传输即将进行时触发。
        /// </summary>
        public Action<FileOperationEventArgs> OnFileTransfering { get; set; }
                    ...
                    case WaitDataStatus.SetRunning:
                        {
                            ...
                            {
                                return fileOperator.SetFileResult(new Result(ResultCode.Error, ResType.RemoteRefuse.GetDescription()));
                            }
                ...
                if (this.m_eventArgs.TryRemove(waitTransfer.EventHashCode, out FileOperationEventArgs args))
                {
                    try
                    {
                        using (FileStorageReader reader = FilePool.GetReader(waitTransfer.Path))
                        {
                            ...
                            {
                                ByteBlock byteBlock = BytePool.GetByteBlock(1024 * 512);
                                ...
                                try
                                {
                                    ...
                                    while (true)
                                    {
                                        ...
                                        int r = reader.Read(byteBlock.Buffer, 0, byteBlock.Capacity);
                                        if (r == 0)
                                        {
                                            channel.Complete();
                                            break;
