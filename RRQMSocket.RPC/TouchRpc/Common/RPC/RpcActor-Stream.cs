...
namespace RRQMSocket.RPC.TouchRpc
{
    public partial class RpcActor
    {
        ...
        public Result SendStream(Stream stream, StreamOperator streamOperator, Metadata metadata = default)
        {
            ...
            int length = TouchRpcUtility.TransferPackage;
        ...
        private void P_8_RequestStreamToThis(WaitStream waitStream)
        {
            ...
            try
            {
                this.OnStreamTransfering?.Invoke(args);
                if (args.Operation.HasFlag(Operation.Permit))
                {
                    ...
                    else
                    {
                        ...
                        this.OnStreamTransfered?.Invoke(new StreamStatusEventArgs(new Result(ResultCode.Error, waitStream.Message), waitStream.Metadata, streamInfo)
                ...
                else
                {
                    ...
                    this.OnStreamTransfered?.Invoke(new StreamStatusEventArgs(new Result(ResultCode.Error, ResType.RemoteRefuse.GetDescription(waitStream.Message)), waitStream.Metadata, streamInfo)
                    { Message = args.Message });
                }
            }
            catch (Exception ex)
        ...
        private void TaskTransferingStream(StreamOperator streamOperator, StreamOperationEventArgs args, Channel channel, long size)
        {
            Task.Run(() =>
            {
                ...
                this.OnStreamTransfered?.Invoke(new StreamStatusEventArgs(streamOperator.SetStreamResult(new Result(channel.Status.ToResultCode())),
