...
namespace RRQMSocket.RPC.TouchRpc
{
    ...
    public partial class RpcActor : DisposableObject, IRpcActor, IInternalRpc
    {
        ...
        /// 在完成握手连接时
        /// </summary>
        public Action<MesEventArgs> OnHandshaked { get; set; }

        /// <summary>
        /// 握手
        /// </summary>
        public Action<VerifyOptionEventArgs> OnHandshaking { get; set; }

        /// <summary>
        /// 接收到数据
        /// </summary>
        public Action<short, ByteBlock> OnReceived { get; set; }

        /// <summary>
        /// 重设ID
        /// </summary>
        public Action<WaitSetID> OnResetID { get; set; }

        /// <summary>
        /// 流数据处理，用户需要在此事件中对e.Bucket手动释放。
        /// </summary>
        public Action<StreamStatusEventArgs> OnStreamTransfered { get; set; }

        /// <summary>
        /// 即将接收流数据，用户需要在此事件中对e.Bucket初始化。
        /// </summary>
        public Action<StreamOperationEventArgs> OnStreamTransfering { get; set; }

        /// <summary>
        /// 发送数据接口
        /// </summary>
        public Action<bool, TransferByte[]> OutputSend { get; set; }
        ...
        /// 日志
        /// </summary>
        public ILog Logger => this.m_logger;
        ...
        /// 构造函数
        /// </summary>
        public RpcActor(bool isLocal, ILog logger)
        {
            ...
            this.m_waitCallback_InvokeThis = InvokeThis;
        ...
        public void Handshake(string verifyToken, CancellationToken token = default, int timeout = 5000)
        {
            if (this.m_isHandshaked)
            {
                return;
                    ...
                    case WaitDataStatus.SetRunning:
                        {
                            ...
                            if (verifyResult.Status == 1)
                            {
                                ...
                                this.OnHandshaked(new MesEventArgs("成功建立"));
                ...
                case TouchRpcUtility.P_0_Handshake_Request:
                    {
                        try
                        {
                            ...
                            this.OnHandshaking?.Invoke(args);
                            ...
                            if (args.Operation.HasFlag(Operation.Permit))
                            {
                                ...
                                this.m_isHandshaked = true;
                                this.OnHandshaked?.Invoke(new MesEventArgs("成功建立"));
                            }
                            else
                            {
                                waitVerify.Status = 2;
                ...
                case TouchRpcUtility.P_1_ResetID_Request:
                    {
                        try
                        {
                            ...
                            try
                            {
                                this.OnResetID?.Invoke(waitSetID);
                ...
                case TouchRpcUtility.P_1002_Ping_Response://心跳
                    {
                        try
                        {
                            this.WaitHandlePool.SetRun(RRQMBitConverter.Default.ToInt64(byteBlock.Buffer, 2));
                        }
                        catch (Exception ex)
                        {
        ...
        public bool Ping(int timeout = 5000)
        {
            try
            {
                ...
                this.SocketSend(TouchRpcUtility.P_2_Ping_Request, RRQMBitConverter.Default.GetBytes(wait.WaitResult.Sign));
        ...
        public void ResetID(string id, CancellationToken cancellationToken = default)
        {
            ...
            switch (waitData.Wait(5000))
            {
                ...
                case WaitDataStatus.Disposed:
                default:
                    throw new RRQMException(ResType.UnknownError.GetDescription());
            }
        }
        ...
        protected override void Dispose(bool disposing)
        {
            ...
            this.WaitHandlePool.SafeDisposeWithNull();
        ...
        public void Send(short protocol, byte[] buffer, int offset, int length)
        {
            ...
            {
                throw new RRQMException("小于0的协议为内部协议。");
            }
        }
        ...
        private void SocketSend(short protocol, byte[] dataBuffer, int offset, int length)
        {
            ...
            this.OutputSend?.Invoke(false, transferBytes);
        }
        ...
        private void SocketSendAsync(short protocol, byte[] dataBuffer, int offset, int length)
        {
            ...
            this.OutputSend?.Invoke(true, transferBytes);
        }
